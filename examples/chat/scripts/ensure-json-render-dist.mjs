...
/**
 * Builds workspace package dist outputs required by this example.
 */
function ensureWorkspaceBuilds() {
  const missing = requiredPackages.filter((pkg) => !existsSync(resolve(exampleRoot, pkg.entry)));
  if (missing.length === 0) return;

  for (const pkg of missing) {
    const result = spawnSync("pnpm", ["--dir", repoRoot, "--filter", pkg.name, "build"], {
      stdio: "inherit",
      env: process.env,
    });
